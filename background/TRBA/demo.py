...
def demo(opt):
    ...
    model.eval()
    with torch.no_grad():
        log = open(f"./TRBA/log_demo_result.txt", "a")
...
def get_onmtpname(pics_name, picnum, onmtpnum, txt="TRBA/log_demo_result.txt"):
    with open(txt, "r") as f:
        lines = f.readlines()

    for line in lines[::-1]:
        if f"{pics_name[picnum][:-4]}_{onmtpnum}." in line:
            onmtp_name = re.findall("\t(.*?) ", line)
            break
        else:
            onmtp_name = "取得できません"
...
def main(pics_name, picnum, onmtpnum):
    ...
    onmtp_name = get_onmtpname(pics_name, picnum, onmtpnum)
    logger.info(f"log of line 334: onmtp_name = {onmtp_name}")
...
if __name__ == "__main__":
    main(pics_name="y_63ec7ad5931e1_19.jpg")
